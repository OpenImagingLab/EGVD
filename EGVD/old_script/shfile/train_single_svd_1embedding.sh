...
accelerate launch --mixed_precision="fp16" evs_train_single_svd_1embedding.py \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --variant "fp16" \
  --num_frames 14 \
  --train_data_dir=$TRAIN_DIR \
  --validation_data_dir=$VALIDATION_DIR \
  --max_train_samples=144 \
  --train_batch_size=1 \
  --gradient_accumulation_steps 1 \
  --num_train_epochs=800 --checkpointing_steps=500 \
  --validation_epochs=50 \
  --learning_rate=1e-04 --lr_scheduler="constant" --lr_warmup_steps=0 \
  --seed=42 \
  --double_sampling_rate \
  --output_dir="ckpt/checkpoints_single_svd_1embedding/svd_reverse_motion_with_attnflip" \
  --cache_dir="ckpt/checkpoints_single_svd_1embedding/svd_reverse_motion_with_attnflip_cache" \
