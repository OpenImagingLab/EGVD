...
from custom_diffusers.pipelines.evs_pipeline_frame_interpolation_with_noise_injection_1embedding import EVSFrameInterpolationWithNoiseInjectionPipelineV2 as EVSFrameInterpolationWithNoiseInjectionPipeline
# from custom_diffusers.pipelines.evs_pipeline_frame_interpolation_with_noise_injection import EVSFrameInterpolationWithNoiseInjectionPipeline
...
def tensor_to_pillow(tensor, save_path):
    ...
    image_data = tensor.squeeze(0).permute(1, 2, 0).cpu().numpy()
...
def main(args):
    ...
    dataset = StableVideoTestDataset(args.frames_dirs,num_frames=finetuned_unet.config.num_frames)
