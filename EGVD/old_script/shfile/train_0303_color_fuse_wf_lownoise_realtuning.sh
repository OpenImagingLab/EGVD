...
accelerate launch --mixed_precision="fp16" evs_train_single_svd3_color_dlc_wf_lownoise_0303.py \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --variant "fp16" \
  --num_frames 9 \
  --train_data_dir=$TRAIN_DIR \
  --validation_data_dir=$VALIDATION_DIR \
  --max_train_samples=220 \
  --train_batch_size=1 \
  --gradient_accumulation_steps 1 \
  --num_train_epochs=600 --checkpointing_steps=5000 \
  --validation_epochs=50 \
  --learning_rate=1e-04 --lr_scheduler="constant" --lr_warmup_steps=0 \
  --seed=42 \
  --skip_sampling_rate=-1 \
  --output_dir="ckpt/checkpoints_z0303_newfusemodel_lownoise_real/svd_reverse_motion_with_attnflip" \
  --cache_dir="ckpt/checkpoints_z0303_newfusemodel_lownoise_real/svd_reverse_motion_with_attnflip_cache" \
