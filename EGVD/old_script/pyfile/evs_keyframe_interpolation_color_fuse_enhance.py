...
from custom_diffusers.pipelines.evs_pipeline_frame_interpolation_with_noise_injection_color_fuse_enhance import EVSFrameInterpolationWithNoiseInjectionPipeline
...
def main(args):
    ...
    lfnet_path = args.checkpoint_dir + "/lfnet_checkpoint.pt"
    print("load_lef_model:",lfnet_path)
    lfnet_state_dict = torch.load(lfnet_path, map_location='cpu')
    # print(lfnet_state_dict)
    lefnet_new_state_dict = {}
    for k, v in lfnet_state_dict.items():
        new_key = k[7:] if k.startswith('module.') else k  # Remove 'module.' prefix
        lefnet_new_state_dict[new_key] = v
    pipe.enhance.load_state_dict(lefnet_new_state_dict)   
