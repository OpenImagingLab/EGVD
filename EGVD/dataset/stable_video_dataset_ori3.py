...
def great_filter(event_image, kernel_size=3, iterations=1):
    event_image = event_image.astype(np.float32)
    event_image = event_image/np.max(event_image)
    # event_image = cv2.GaussianBlur(event_image, (0, 0), sigma_log)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
    dilated_image = cv2.dilate(event_image, kernel, iterations=iterations)
    eroded_image = cv2.erode(dilated_image, kernel, iterations=iterations)
    eroded_image = np.clip(eroded_image, 0, 1)
    return eroded_image

def create_event_image(args,x, y, p, t, shape, B=3):
    ...
    if args.event_filter == "great_filter":
        print("great_filtergreat_filtergreat_filtergreat_filtergreat_filtergreat_filter")
        return great_filter(event_image)
    else:
