...
accelerate launch --mixed_precision="fp16" evs_train_single_svd3_color.py \
  --pretrained_model_name_or_path=$MODEL_NAME \
  --variant "fp16" \
  --num_frames 9 \
  --train_data_dir=$TRAIN_DIR \
  --validation_data_dir=$VALIDATION_DIR \
  --max_train_samples=385 \
  --train_batch_size=1 \
  --gradient_accumulation_steps 1 \
  --num_train_epochs=520 --checkpointing_steps=10000 \
  --validation_epochs=50 \
  --learning_rate=1e-04 --lr_scheduler="constant" --lr_warmup_steps=0 \
  --seed=42 \
  --skip_sampling_rate=-1 \
  --output_dir="ckpt/checkpoints_wnorm_wofilter_v8_color3_mixskip2/svd_reverse_motion_with_attnflip" \
  --cache_dir="ckpt/checkpoints_wnorm_wofilter_v8_color3_mixskip2/svd_reverse_motion_with_attnflip_cache" \
