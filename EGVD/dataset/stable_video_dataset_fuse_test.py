...
def save_debug_images_as_rgb(debug_dir, event_image, cumulative_image, B):
    ...
    for b in range(B):
        ...
        cv2.imwrite(os.path.join(debug_dir, f"muti_{b}.png"), cumulative_channel_rgb*event_channel_rgb)
 
 
def create_event_image(args, x, y, p, t, shape, B, rep_n,debug=True):
    ...
    # print(1111111111111)
    # event_image = cv2.resize(event_image, (512, 512), interpolation=cv2.INTER_LINEAR)
    # cumulative_image =  cv2.resize(cumulative_image, (512, 512), interpolation=cv2.INTER_LINEAR)
    print(22222222222)

    if args.event_filter == "great_filter":
        for i in range(0, B, rep_n):  # 步长为3，处理每组三个通道
 
            mask = (bin_idx <= i+rep_n) & (bin_idx >= np.maximum(0, i))
            cumulative_image_f = cumulative_image[:, :, 0].copy()
            np.add.at(cumulative_image_f, (y[mask], x[mask]), np.abs(p[mask]))
            motion_mask = mask_function(cumulative_image_f)
            for j in range(i, min(i + rep_n, B)):
                cumulative_image[:, :, j] = motion_mask.astype(np.uint8)
        if debug:
            save_debug_images_as_rgb("./debug_output", event_image, cumulative_image, B)
        cumulative_image[np.isnan(cumulative_image)] = 1
...
class StableVideoDataset(Dataset):
    def __init__(self, 
        ...
        video_names = ["a_sim2real_haze_20lux"]
        ...
        rep_n = 8
        self.pixel_transforms_evs = transforms.Compose([
            transforms.Normalize(mean=[0.5] * rep_n, std=[0.5] * rep_n, inplace=True),
        ])
    ...
    def accumulate_events(self, video_frame_paths, insertion_rate_iter):
        ...
        rep_n = 8 
        event_voxel_bin = create_event_image(self.args, x_all, y_all, p_all, t_all, shape, (insertion_rate_iter-1) * rep_n, rep_n)
        ...
        event_voxel_bin = event_voxel_bin.view((insertion_rate_iter-1), rep_n, event_voxel_bin.shape[-2], event_voxel_bin.shape[-1])
        ...
        event_voxel_bin = torch.cat([event_voxel_bin[0:1].clone(), event_voxel_bin], dim=0)
    ...
    def __getitem__(self, idx):
        if self.skip_sampling_rate<1:
            ...
            skip_rate = random.choices([1, 2, 3], weights=[0.2, 0.3, 0.5], k=1)[0]
        ...
        event_voxel_bin = self.pixel_transforms_evs(event_voxel_bin)
...
import argparse
# 定义一个伪参数类，包含必要的属性（例如 event_filter）
class DummyArgs:
    event_filter = "great_filter"  # 可选 "great_filter" 或其他

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="测试 StableVideoDataset 和 StableVideoTestDataset")
    parser.add_argument("--data_dir", type=str, default="/mnt/workspace/zhangziran/Self_collected_DATASET/train_sd_vfi/test", help="视频数据所在的文件夹路径")
    parser.add_argument("--max_num_videos", type=int, default=1, help="测试时使用的视频数")
    parser.add_argument("--test_mode", action="store_true", help="是否使用测试数据集 StableVideoTestDataset")
    args = parser.parse_args()

    dummy_args = DummyArgs()

    # 根据参数选择使用训练集数据集或测试数据集
    if args.test_mode:
        dataset = StableVideoTestDataset(dummy_args, args.data_dir, max_num_videos=args.max_num_videos)
    else:
        dataset = StableVideoDataset(dummy_args, args.data_dir, max_num_videos=args.max_num_videos)
    ...
    print("Pixel values 的形状:", sample["pixel_values"].shape)
    print("Event voxel bin 的形状:", sample["event_voxel_bin"].shape)
    
    # 如果是测试数据集，则还会有保存名称
    if args.test_mode and "save_name" in sample:
        print("保存名称:", sample["save_name"])