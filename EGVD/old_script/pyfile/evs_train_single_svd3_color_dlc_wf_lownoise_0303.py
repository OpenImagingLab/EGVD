...
from dataset.stable_video_dataset_fuse_real import StableVideoDataset
...
def main():
    ...
    unet_path = "/mnt/workspace/zhangziran/DiffEVS/svd_keyframe_interpolation-main/ckpt/checkpoints_z0220_newfusemodel_lownoise/svd_reverse_motion_with_attnflip/checkpoint-60000"
    # unet = UNetSpatioTemporalConditionModel.from_pretrained(
    #     unet_path, subfolder="unet",torch_dtype=torch.float16, low_cpu_mem_usage=True, variant=args.variant
    # )
    unet = UNetSpatioTemporalConditionModel.from_pretrained(
        unet_path,
        subfolder="unet",
    ...
    pre_path = unet_path + "/lef_model_checkpoint.pt"
    lef_model = initialize_or_load_weights(lef_model,pretrained_path=pre_path)
    ...
    # )
    unet, lef_model, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        unet, lef_model, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(lef_model):
                ...
                target = latents
                mse_loss_latents = torch.mean(
                    (new_condition.float() - target.float()) ** 2
                )
                if epoch > 100:
                    ...
                    loss_svd = loss.mean()
                    loss = loss_svd #+ 0.05 * mse_loss_latents
                else:
                    loss = mse_loss_latents
