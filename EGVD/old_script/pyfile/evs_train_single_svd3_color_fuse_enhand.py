...
from dataset.stable_video_dataset_fuse import StableVideoDataset
...
def initialize_or_load_weights(lef_model, pretrained_path=None):
    print("res:",pretrained_path)
    if pretrained_path and os.path.exists(pretrained_path):
        state_dict = torch.load(pretrained_path, map_location='cpu')
        lef_model.load_state_dict(state_dict)
    else:
        for name, param in lef_model.named_parameters():
...
def main():
    ...
    # unet_path = "/mnt/workspace/zhangziran/DiffEVS/svd_keyframe_interpolation-main/ckpt/checkpoints_z0214_wfilter_v8_colorfuse_wof_newfusemodel_woadd_enhand/svd_reverse_motion_with_attnflip/checkpoint-60000"
    # unet = UNetSpatioTemporalConditionModel.from_pretrained(
    #     unet_path, subfolder="unet", low_cpu_mem_usage=True, variant=args.variant
    # )
    
    # Instantiate the model
    lef_model = LatentEventFusion(in_channels=4, out_channels=4)
    lef_model = initialize_or_load_weights(lef_model,"/mnt/workspace/zhangziran/DiffEVS/svd_keyframe_interpolation-main/ckpt/checkpoints_z0214_wfilter_v8_colorfuse_wof_newfusemodel_woadd_enhand/svd_reverse_motion_with_attnflip/checkpoint-60000/lef_model_checkpoint.pt")
    lef_model.to(unet.device)
    lef_model.requires_grad_(True)
    
    
    lfnet = LatentFedNet(in_channels=4, out_channels=4)
    lfnet = initialize_or_load_weights(lfnet)
    lfnet.to(unet.device)
    lfnet.requires_grad_(True)
    # for param in lef_model.parameters():
    #     print(f"{param.requires_grad}")  
    #     param.requires_grad = True  
    


    print("unet.config.num_frames:",unet.config.num_frames,"args.num_frames:",args.num_frames)
    ...
    for name, para in lef_model.named_parameters():
        unet_train_params_list.append(para)
        para.requires_grad = True

    for name, para in lfnet.named_parameters():
        unet_train_params_list.append(para)
        para.requires_grad = True
    ...
    if args.mixed_precision == "fp16":
        ...
        cast_training_params(lef_model, dtype=torch.float32)
        cast_training_params(lefnet, dtype=torch.float32)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                for i, model in enumerate(models):
                    if isinstance(model, LatentEventFusion):
                        lef_model_save_path = os.path.join(output_dir, "lef_model_checkpoint.pt")
                        torch.save(model.state_dict(), lef_model_save_path)
                    elif isinstance(model, LatentFedNet):
                        lfnet_save_path = os.path.join(output_dir, "lfnet_checkpoint.pt")
                        torch.save(model.state_dict(), lfnet_save_path)
                    else:
        ...
        def load_model_hook(models, input_dir):
            for _ in range(len(models)):
                ...
                model = models.pop()
                if isinstance(model, LatentEventFusion):
                    # Load the LEF model state_dict
                    lef_model_load_path = os.path.join(input_dir, "lef_model_checkpoint.pt")
                    model.load_state_dict(torch.load(lef_model_load_path))
                elif isinstance(model, LatentFedNet):
                    lfnet_load_path = os.path.join(input_dir, "lfnet_checkpoint.pt")
                    model.load_state_dict(torch.load(lfnet_load_path))
                else:
        ...
        #     if accelerator.is_main_process:
        #         for i, model in enumerate(models):
        #             # Save the unet model with its own save method
        #             if isinstance(model, UNetSpatioTemporalConditionModel):
        #                 model.save_pretrained(os.path.join(output_dir, "unet"))
        #             # Save the LEF model using torch.save (custom saving method)
        #             elif isinstance(model, LatentEventFusion):
        #                 lef_model_save_path = os.path.join(output_dir, "lef_model_checkpoint.pt")
        #                 torch.save(model.state_dict(), lef_model_save_path)
                        
        #             # Make sure to pop weights so the model is not saved again
        #             weights.pop()
    ...
    train_dataset = StableVideoDataset(args=args,
                                       video_data_dir=args.train_data_dir, 
                                       max_num_videos=args.max_train_samples, 
                                       num_frames=args.num_frames,
                                       is_reverse_video=True,
                                       skip_sampling_rate=args.skip_sampling_rate)
    def collate_fn(examples):
        ...
        if "event_voxel_bin" in examples[0].keys():
            event_voxel_bin = torch.stack([example["event_voxel_bin"] for example in examples])
            event_voxel_bin = event_voxel_bin.to(memory_format=torch.contiguous_format).float()
            return {
                "pixel_values": pixel_values,
                "conditions": conditions,
                "event_voxel_bin": event_voxel_bin
            }
        else:
    ...
    # )
    unet, lef_model,lfnet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        unet, lef_model,lfnet, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet), accelerator.accumulate(lef_model), accelerator.accumulate(lfnet):
                # Get the image embedding for conditioning
                encoder_hidden_states = compute_image_embeddings(batch["conditions"])
                encoder_hidden_states_ref = compute_image_embeddings(batch["pixel_values"][:, 0])
                ...
                conditions_ref = batch["pixel_values"][:, 0] + noise_aug_strength * noise
                ...
                batch["event_voxel_bin"] = batch["event_voxel_bin"].to(accelerator.device).to(dtype=weight_dtype)
                evs = batch["event_voxel_bin"] 
                # evs_values = rearrange(evs, "b f c h w -> (b f) c h w")
                # evs_latents = vae.encode(evs_values).latent_dist.mode()
                # evs_latents = rearrange(evs_latents, "(b f) c h w -> b f c h w", f=args.num_frames)
                evs_latents = evs
                ...
                #                 print(f"Gradient for {name}: {param.grad.abs().mean():.6f}", "param.requires_grad", param.requires_grad)
                #                 print(f"Weight for {name}: {param.data.abs().mean():.6f}", "param.requires_grad", param.requires_grad)
                #             else:
                #                 print(f"Warning: {name} has no gradient!", "param.requires_grad", param.requires_grad)
                # print_grads(lef_model)
                ...
                if epoch >=0:
                    ...
                    # lambda_value = 1.0  # You can adjust this lambda value as needed
                    ...
                    model_pred = unet(noisy_latents_inp, timesteps,
                                    encoder_hidden_states=encoder_hidden_states_fused, 
                    ...
                    if epoch<200:
                        ...
                        loss = loss_svd #+ mse_loss_latents
                    else:
                        enh_latents = model_pred.clone().detach()
                        lf_latents = lfnet(enh_latents)
                        loss_lf_latents = torch.mean((lf_latents.float() - target.float()) ** 2)
                        loss = loss_lf_latents
                else:
                    loss_latents = torch.mean((new_condition.float() - target.float()) ** 2)
                    loss = loss_latents
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        lef_model_save_path = os.path.join(args.output_dir, f"lef_model_checkpoint-{global_step}.pt")
                        torch.save(lef_model.state_dict(), lef_model_save_path)
                        logger.info(f"Saved lef_model weights to {lef_model_save_path}")

                        lfnet_save_path = os.path.join(args.output_dir, f"lfnet_checkpoint-{global_step}.pt")
                        torch.save(lfnet.state_dict(),  lfnet_save_path)
                        logger.info(f"Saved lfnet weights to {lfnet_save_path}")
    ...
    if accelerator.is_main_process:
        ...
        # Now save the `lef_model` weights to a separate file
        lef_model_save_path = os.path.join(args.output_dir, "lef_model_checkpoint_last.pt")
        torch.save(lef_model.state_dict(), lef_model_save_path)

        lfnet_save_path = os.path.join(args.output_dir, "lfnet_checkpoint_last.pt")
        torch.save(lfnet.state_dict(), lfnet_save_path)

        # Optionally log the save locations
