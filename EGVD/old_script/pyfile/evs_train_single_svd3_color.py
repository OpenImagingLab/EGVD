    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                w_cond = w_cond.view(1, T, 1, 1, 1).expand(B, T, C, H, W).to(device)
                w_ref = w_ref.view(1, T, 1, 1, 1).expand(B, T, C, H, W).to(device)
                w_evs = w_evs.view(1, T, 1, 1, 1).expand(B, T, C, H, W).to(device)
                ...
                # print("beforeshape:",evs_latents.shape,conditions_latent.shape,conditions_latent_ref.shape,new_condition.shape)

                noisy_latents_inp = torch.cat([noisy_latents_inp, new_condition], dim=2)
